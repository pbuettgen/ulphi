...
namespace boost { namespace parameter { namespace python { namespace aux
      {
        ...
        struct UnspecifiedType {
          ...
          {
#if PY_MAJOR_VERSION > 2
            Py_TYPE(&unspecified_) = &PyType_Type;
#else
            unspecified_.ob_type = &PyType_Type;
#endif
            PyType_Ready(&unspecified_);
          }

          PyObject* Get()
          {
            return (PyObject*) &unspecified_;
          }

        private:
          PyTypeObject unspecified_;
        };

        inline PyObject* unspecified_type()
        {
          static UnspecifiedType unspecified;

          return unspecified.Get();
        }
